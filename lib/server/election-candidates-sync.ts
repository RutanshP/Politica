...
export async function syncElectionCandidatesFromFec(options?: ElectionCandidatesSyncOptions) {
  ...
  const [houseCandidates, senateCandidates, presidentCandidates, fecIdsByBioguide] = await Promise.all([
  ...
  const rows = [
    ...
    ...presidentCandidates.map((candidate) => toRow(candidate, presidentialCycle, bioguideByFecId)),
